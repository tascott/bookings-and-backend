...
export default function ServiceAvailabilityManagement({
	...
	const handleEditFieldChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
		...
		if (type === 'checkbox') {
			...
			if (name === 'is_active') {
				...
				const isoDay = parseInt(name.split('-')[1], 10);
				if (!isNaN(isoDay)) {
					setEditFields((prev) => {
						const currentDays = prev.days_of_week.map((d) => parseInt(d, 10));
						if (checked) {
							return {
								...prev,
								days_of_week: [...new Set([...currentDays, isoDay])].map((d) => d.toString()).sort(),
							};
						} else {
							return {
								...prev,
								days_of_week: currentDays
									.filter((day) => day !== isoDay)
									.map((d) => d.toString())
									.sort(),
							};
						}
	...
	const handleSaveEdit = async () => {
		...
		try {
			...
			const daysOfWeekISO = editFields.days_of_week.map((d) => parseInt(d, 10)).filter((d) => !isNaN(d) && d >= 1 && d <= 7);

			const isRecurring = daysOfWeekISO.length > 0;
			const specificDateValue = editFields.specific_date || null;
			...
			const updateData: Partial<Omit<ServiceAvailability, 'id' | 'created_at'>> = {
				...
				days_of_week: isRecurring ? daysOfWeekISO.sort((a, b) => a - b) : null,
				specific_date: !isRecurring ? specificDateValue : null,
				override_price: overridePrice,
			};

			if (!updateData.start_time || !updateData.end_time || updateData.end_time <= updateData.start_time)
				throw new Error('Valid Start and End time required (End must be after Start).');
			if (isRecurring && specificDateValue) throw new Error('Cannot set both recurring days and specific date.');
			if (!isRecurring && !specificDateValue) throw new Error('Must set either Recurring Days or a Specific Date.');
			...
			const errorMsg = e instanceof Error ? e.message : 'Validation failed or unknown error';
			console.error('[handleSaveEdit] Error:', errorMsg, e);
	...
	const availabilityMgmtTabs = [
						...
						<ul style={{ listStyle: 'none', padding: 0 }}>
							{serviceAvailability.map((rule) => {
								...
								return (
									...
									>
										<div>
											<strong>Service:</strong> {getServiceName(rule.service_id)} (Rule ID: {rule.id})
										</div>
										<div>
											<strong>Status:</strong> {rule.is_active ? 'Active' : 'Inactive'}
										</div>
										<div>
										...
										<div>
											...
											{rule.override_price !== null && rule.override_price !== undefined
												...
												: ` Default: ${
														defaultServicePrice !== null && defaultServicePrice !== undefined
															? `£${defaultServicePrice.toFixed(2)}`
															: 'Not set'
												  }`}
										</div>
										<div style={{ marginTop: '0.8rem' }}>
											<button
												onClick={() => handleToggleServiceAvailabilityActive(rule.id, rule.is_active)}
												style={{ marginRight: '0.5rem' }}
												className={`button small ${rule.is_active ? 'secondary' : 'primary'}`}
											>
											...
											<button onClick={() => handleDeleteServiceAvailability(rule.id)} className="button danger small">
												Delete
											</button>
										</div>
									</li>
		...
		},
		{
			...
			content: (
				<>
					<form
						ref={addServiceAvailabilityFormRef}
						onSubmit={handleAddServiceAvailability}
						...
						{services.length === 0 ? (
							...
							<>
												...
												{getFieldsForSite(site.id).length > 0 ? (
													getFieldsForSite(site.id).map((field) => (
														<label key={`avail-field-${field.id}`} style={{ display: 'block', marginLeft: '1rem' }}>
															<input type="checkbox" name="availabilityFieldIds" value={field.id} />{' '}
								...
								<div style={{ border: '1px dashed #ccc', padding: '0.5rem', marginBottom: '0.5rem' }}>
									...
									<div style={{ marginLeft: '1rem', marginTop: '0.5rem' }}>
										...
										{['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, index) => {
											...
											return (
												<label key={isoDay} style={{ marginRight: '0.5rem' }}>
													<input
														type="checkbox"
														name={`availabilityDayOfWeek-${isoDay}`}
														value={isoDay.toString()}
														checked={editFields.days_of_week.includes(isoDay.toString())}
									...
									<div style={{ marginLeft: '1rem' }}>
										<label>
											...
											<span style={{ fontSize: '0.8em', color: '#777' }}>
												(Only check if capacity should be limited by assigned staff&apos;s vehicle)
											</span>
										</label>
									</div>
								</div>
								...
								<div style={{ marginTop: '0.5rem' }}>
									...
									<input
										type="number"
										id="availabilityOverridePrice"
										name="availabilityOverridePrice"
										min="0"
										step="0.01"
								...
								<button type="submit" style={{ marginTop: '1rem' }} className="button primary">
									Add Rule
								</button>
							</>
						)}
					...
					>
						...
						<div style={{ border: '1px dashed #ccc', padding: '0.5rem', marginBottom: '0.5rem' }}>
							...
							<div style={{ marginLeft: '1rem' }}>
								...
								<input
									type="date"
									name="specific_date"
									value={editFields.specific_date}
									onChange={handleEditFieldChange}
									className="input"
						...
						<div style={{ marginBottom: '1rem' }}>
							...
							<input
								type="number"
								name="override_price"
								value={editFields.override_price}
								onChange={handleEditFieldChange}
								min="0"
								step="0.01"
						...
						<div style={{ marginTop: '1rem', display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
							<button onClick={closeEditModal} disabled={isSaving} className="button secondary">
								Cancel
							</button>
							<button onClick={handleSaveEdit} disabled={isSaving} className="button primary">
